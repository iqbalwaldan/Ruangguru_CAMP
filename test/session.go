...
func (u *sessionsRepoImpl) SessionAvailName(name string) error {
	...
	err := u.db.QueryRow("SELECT id, token, username, expiry FROM sessions WHERE username = $1 LIMIT 1", name).Scan(&session.ID, &session.Token, &session.Username, &session.Expiry)
	if err != nil {
